...
function timelineLabel(exp: { status: ExperienceStatus; period: string }): string {
  if (exp.status === 'current') return 'Current'
  const m = exp.period.match(/(\w+)\s+(\d{4})\s*[—–-]/)
  if (!m) return exp.period
  const [, month, year] = m
  const shortYear = `'${year.slice(2)}`
  // May–Aug → Summer
  if (['May', 'June', 'Jul', 'Aug'].includes(month)) return `Summer ${shortYear}`
          ...
          {featured_experiences.map((exp, idx) => (
            <div key={`${exp.company}-${exp.title}`} className="flex items-center">
              ...
              <div className="flex flex-col items-center">
                ...
                <span className="mt-2 text-xs font-medium tracking-wide text-gray-400 dark:text-gray-500">
                  {timelineLabel(exp)}
                </span>
              </div>
